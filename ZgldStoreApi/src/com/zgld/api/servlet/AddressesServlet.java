...
/**
 * Servlet implementation class AddressesServlet
 */
@WebServlet("/json/addresses/*")
public class AddressesServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (currentPath.length() >= 6) {
			int index = currentPath.indexOf("/json/addresses") + 16;
			int last = currentPath.indexOf(".html");
			if (index > 0 && last > 0) {
				String id = currentPath.substring(index, last);
				getServletContext().getRequestDispatcher("/addresses_" + id + ".do").forward(request, response);
			} else {
