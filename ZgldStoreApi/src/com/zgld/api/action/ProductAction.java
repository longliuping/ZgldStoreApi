...
public class ProductAction extends BaseAction {
	...
	public String home_type_product() {
		...
		try {
			if ((this.form.getId() == null) || (this.form.getId().intValue() <= 0)) {
				...
				List listProducts = this.baseBiz.findPage(this.form.getPageNum().intValue(),
	...
	public String product_detail() {
		...
		try {
			if (this.form.getId() == null) {
				...
				Products product = (Products) this.baseBiz.bean(" from Products as p where p.productId = " + productId);
				if (product == null) {
					this.form.setJsonMsg("产品不存在", false, json, 1001);
				} else {
					List listImages = this.baseBiz
							.findAll(" from ProductImages as pi where pi.productId = " + product.getProductId());
					product.setListProductImages(listImages);
					YShop info = (YShop) this.baseBiz.bean(" from YShop as s where s.shopId = " + product.getShopId());
					List<Skugroup> listSkugroups = (List<Skugroup>) this.baseBiz
							.findAll(" from Skugroup as sg where sg.productId = " + productId);
					for (int i = 0; i < listSkugroups.size(); i++) {
						List<Sku> listSku = (List<Sku>) this.baseBiz.findAll(" from Sku as s where s.skugroupId = "
	...
	public String home_all_product() {
		Map json = new HashMap();
		try {
			List listCategories = this.baseBiz.findAll(" from Categories as hc where hc.depth = 1");
			for (int i = 0; i < listCategories.size(); i++) {
				...
				List listInfo = this.baseBiz.findPage(1, 6, hql);
