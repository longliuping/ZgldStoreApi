...
public class ProductAction extends BaseAction {
	...
	public String home_type_product() {
		...
		try {
			...
			// " from HishopProducts as p where p.typeId = " + form.getId() +
	...
	public String product_detail() {
		...
		try {
			...
			// baseBiz.bean(" from HishopProducts as p where p.productId = " +
			// productId);
			// Supplier info = (Supplier)
			// baseBiz.bean(" from Supplier as s where s.userId = " +
			// hishopProducts.getUserid());
			// // 规格
			// List<HishopSkus> hishopSkus = (List<HishopSkus>)
			// baseBiz.findAll(" from HishopSkus as s where s.productId = " +
			// productId);
			// StringBuffer hqlHishopAttributes = new
			// StringBuffer(" from HishopAttributes as ha where ");
			// StringBuffer hqlHishopAttributeValues = new
			// StringBuffer(" from HishopAttributeValues as hav where ");
			// List<HishopSkuitems> listHishopSkuitems = new
			// ArrayList<HishopSkuitems>();
			// for (int i = 0; i < hishopSkus.size(); i++) {
			// HishopSkus hishopSkus2 = hishopSkus.get(i);
			// List<HishopSkuitems> hishopSkuitems = (List<HishopSkuitems>)
			// baseBiz.findAll(" from HishopSkuitems as hsi where hsi.skuId = '"
			...
			// {
			// hqlHishopAttributes.delete(hqlHishopAttributes.toString().length()
			// - 3, hqlHishopAttributes.toString().length());
			// hqlHishopAttributeValues.delete(hqlHishopAttributeValues.toString().length()
			// - 3, hqlHishopAttributeValues.toString().length());
			// List<HishopAttributes> hishopAttributes =
			// (List<HishopAttributes>)
			// baseBiz.findAll(hqlHishopAttributes.toString() +
			// " order by ha.displaySequence desc ");
			// List<HishopAttributeValues> hishopAttributeValues =
			// (List<HishopAttributeValues>)
			// baseBiz.findAll(hqlHishopAttributeValues.toString() +
	...
	public String home_all_product() {
		Map<String, Object> json = new HashMap<String, Object>();
		try {
			// List<HishopCategories> listTypes = (List<HishopCategories>)
			// baseBiz.findAll(" from HishopCategories as hc where hc.depth = 1");
			// for (int i = 0; i < listTypes.size(); i++) {
			// HishopCategories info = listTypes.get(i);
			//
			// List<?> list = baseBiz.findPage(1, 3,
			// " from HishopProducts as hp,HishopCategories as hc where hp.typeId = hc.associatedProductType and hc.categoryId ="
			// + info.getCategoryId() + " order by hp.addedDate asc ");
			// List<HishopProducts> hishopProducts = new
